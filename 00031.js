...
function countProfit(shoppers) {
  let listBarang = [
    { name: "Sepatu Stacattu", price: 1500000, stock: 10, shoppers: [], totalProfit: 0 },
    { name: "Baju Zoro", price: 500000, stock: 2, shoppers: [], totalProfit: 0 },
    { name: "Sweater Uniklooh", price: 175000, stock: 1, shoppers: [], totalProfit: 0 }
  ];
  ...
  for (let i = 0; i < shoppers.length; i++) {
    ...
    let produk = listBarang.find((produk) => produk.name === shopper.produk);

    if (!produk || produk.stock < shopper.amount) {
      continue;
    }

    produk.stock -= shopper.amount;
    produk.shoppers.push(shopper.name);
    produk.totalProfit += shopper.amount * produk.price;
  }

  return listBarang.map((produk) => {
    return {
      produk: produk.name,
      shoppers: produk.shoppers,
      leftOver: produk.stock,
      totalProfit: produk.totalProfit
    };
  });
}
